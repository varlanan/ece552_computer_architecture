...
/* Q1: 5-stage pipeline; Q2: 6-stage pipeline */

/* register counter arrays that keep count of when a reg 
 * will be ready to use safely without the need of stalls */
static counter_t reg_ready_q1[MD_TOTAL_REGS];
static counter_t reg_ready_q2[MD_TOTAL_REGS];

static counter_t sim_num_RAW_hazard_q1_stall1cycle;
static counter_t sim_num_RAW_hazard_q2_stall1cycle;
static counter_t sim_num_RAW_hazard_q1_stall2cycle;
static counter_t sim_num_RAW_hazard_q2_stall2cycle;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q1_stall1cycle",
		   "total number of RAW hazards that cause 1 cycle stall (q1)",
		   &sim_num_RAW_hazard_q1_stall1cycle, sim_num_RAW_hazard_q1_stall1cycle, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_hazard_q1_stall2cycle",
		   "total number of RAW hazards that cause 2 cycle stalls (q1)",
		   &sim_num_RAW_hazard_q1_stall2cycle, sim_num_RAW_hazard_q1_stall2cycle, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2_stall1cycle",
		   "total number of RAW hazards that cause 1 cycle stall (q2)",
		   &sim_num_RAW_hazard_q2_stall1cycle, sim_num_RAW_hazard_q2_stall1cycle, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2_stall2cycle",
		   "total number of RAW hazards that cause 2 cycle stalls (q2)",
		   &sim_num_RAW_hazard_q2_stall2cycle, sim_num_RAW_hazard_q2_stall2cycle, NULL);

  /* CPI = 1 + stall_frequency * stall_cycles 
   * stall_frequency = num_hazard/total_num_insn */
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   "CPI from RAW hazard (q1)",
		   "1 + sim_num_RAW_hazard_q1_stall2cycle/sim_num_insn * 2 \
       + sim_num_RAW_hazard_q1_stall1cycle/sim_num_insn * 1", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + sim_num_RAW_hazard_q2_stall2cycle/sim_num_insn * 2 \
       + sim_num_RAW_hazard_q2_stall1cycle/sim_num_insn * 1", NULL);
  ...
  stat_reg_formula(sdb, "sim_num_use_ratio",
       "load use fraction",
...
sim_main(void)
{
  ...
  /* ECE552 Preassignment - BEGIN CODE */
  // r_out: target registers; r_in: source register numbers
  int r_out[2], r_in[3];
  /* ECE552 Preassignment - END CODE */
...
#include "machine.def"
    ...
    // target regs of LW insn will only be available 2 cycles later
    if( ( MD_OP_FLAGS(op) & F_MEM ) && ( MD_OP_FLAGS(op) & F_LOAD ) ) {
      if( r_out[0] != DNA )
      ...
      for( i = 0; i < 3; i++ ) {
        if( r_in[i] != DNA && reg_ready_q1[r_in[i]] > sim_num_insn ){
          if( reg_ready_q1[r_in[i]] == sim_num_insn + 2 ) {
            sim_num_RAW_hazard_q1_stall2cycle++;
            /* If a 2 cycle hazard occurs, then any next read of the same reg 
             * would not cause any more data stalls; 
             * therefore set it ready in the next insn */
            reg_ready_q1[r_in[i]] = sim_num_insn + 1;
          }
          else  if( reg_ready_q1[r_in[i]] == sim_num_insn + 1 ) 
            sim_num_RAW_hazard_q1_stall1cycle++;

          /* only one hazard per insn */
    ...
    /* 6-stage pipeline */
    {
      ...
      for( i = 0; i < 3; i++ ) {
        if( r_in[i] != DNA && reg_ready_q2[r_in[i]] > sim_num_insn ) {
          /* No need to stall for SW operation if the register involved in the dependency
           * is the register that will be stored into another */
          if( i = 0 && (MD_OP_FLAGS(op) && F_MEM) && (MD_OP_FLAGS(op) && F_STORE) )
            continue;

          if( reg_ready_q2[r_in[i]] - sim_num_insn == 2 ) {
            sim_num_RAW_hazard_q2_stall2cycle++;
            reg_ready_q2[r_in[i]] = sim_num_insn + 1;
          }
          else if ( reg_ready_q2[r_in[i]] - sim_num_insn == 1 )
            sim_num_RAW_hazard_q2_stall1cycle++;
...

    {
      ...
      for ( i = 0; i < 2; i++ ) {
        if( r_out[i] != DNA ) {
          /* If a SW operation occurs, no dependency can be created by its inputs;
           * therefore no stalls, reg ready on next insn */
    ...
    }
    {
      ...
      for ( i = 0; i < 2; i++ ) {
        if( r_out[i] != DNA ) {
          ...
          else reg_ready_q2[r_out[i]] = sim_num_insn + 2;
        }
      }
    }
