...
#define MISS_HIST_SIZE 1024
#define NUM_RPT_ENTRIES_OPEN 1024
...
struct cache_t
{
  ...
  /* ECE552 Assignment 4 - BEGIN CODE */
  struct rpt_entry *rpt_t;
  int rpt_size;

  struct miss_hist_entry *miss_hist_t;
  int miss_hist_head;
  /* ECE552 Assignment 4 - END CODE */
...
enum rpt_state {
  INIT,		  /* set at first entry in RPT or after entry exp diff stride from steady state one */
  STEADY,	  /* prediction stable for a while */
  TRANS,    /* system unsure if current stride is stable */
  NOPRED		/* disable prefetching for this entry for the time being */
};

struct rpt_entry
{
  md_addr_t tag;          /* tag corresponding to addr of mem insn - PC */
  md_addr_t prev_addr;    /* last addr referenced by corresponding insn */
  int stride;             /* diff between last 2 addr gen by corresp PC (can be -ve) */
  enum rpt_state state;   /* four states of the RPT*/
};

struct rpt_entry *rpt_create(int rpt_size);
void stride_update(struct cache_t *cp, md_addr_t addr, int stride);

struct miss_hist_entry
{
  md_addr_t addr;
  int num_occurences;
};
