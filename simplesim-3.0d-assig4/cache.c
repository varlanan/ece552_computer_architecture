...
struct rpt_entry *rpt_create(int rpt_size) 
{
  int rpt_idx;
  struct rpt_entry *rpt;
  rpt = (struct rpt_entry *)calloc(rpt_size, sizeof(struct rpt_entry));
  assert(rpt);
  for( rpt_idx = 0; rpt_idx < rpt_size; rpt_idx++ ){
    rpt[rpt_idx].tag = 0;
    rpt[rpt_idx].prev_addr = 0;
    rpt[rpt_idx].stride = 0;
    rpt[rpt_idx].state = INIT;
  }
  return rpt;
}

struct miss_hist_entry *miss_hist_create(int miss_hist_t_size)
{
  int miss_hist_idx;
  struct miss_hist_entry *miss_hist;
  miss_hist = (struct miss_hist_entry *)calloc(miss_hist_t_size, sizeof(struct miss_hist_entry));
  assert(miss_hist);
  for( miss_hist_idx = 0; miss_hist_idx < miss_hist_t_size; miss_hist_idx++ ){
    miss_hist[miss_hist_idx].addr = 0;
    miss_hist[miss_hist_idx].num_occurences = 0;
  }
  return miss_hist;
}
	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  /* ECE552 Assignment 4 - BEGIN CODE */
  cp->rpt_size = prefetch_type;
  cp->rpt_t = NULL;

  cp->miss_hist_t = NULL;
  cp->miss_hist_head = 0;
  //cp->miss_hist_size = MISS_HIST_SIZE;

  if( cp->prefetch_type == 2 ) {
    cp->rpt_t = rpt_create(NUM_RPT_ENTRIES_OPEN);
    assert(cp->rpt_t);
    cp->rpt_size = NUM_RPT_ENTRIES_OPEN;

    cp->miss_hist_t = miss_hist_create(MISS_HIST_SIZE);
    assert(cp->miss_hist_t);
  }
  else if( cp->prefetch_type > 2 ) {
    cp->rpt_t = rpt_create(cp->prefetch_type);
    assert(cp->rpt_t);
    cp->rpt_size = cp->prefetch_type;
  }
...
/* ECE552 Assignment 4 - BEGIN CODE */
void stride_update(struct cache_t *cp, md_addr_t addr, int stride) {
  md_addr_t prefetch_addr = addr + stride;
  md_addr_t prefetch_tag = CACHE_TAG(cp, prefetch_addr);
  md_addr_t prefetch_set = CACHE_SET(cp, prefetch_addr);

  prefetch_addr = CACHE_MK_BADDR(cp, prefetch_tag, prefetch_set);

  if(!cache_probe(cp, prefetch_addr))
    cache_access(cp,	Read,	prefetch_addr, NULL, cp->bsize,	0, NULL, NULL,	1);
}

void miss_hist_prefetch(struct cache_t *cp, md_addr_t addr, int miss_hist_idx) {
  int miss_hist_pref_idx = (miss_hist_idx + 1)%MISS_HIST_SIZE;
  md_addr_t prefetch_addr = cp->miss_hist_t[miss_hist_pref_idx].addr;
  if(prefetch_addr != 0) {
    md_addr_t prefetch_tag = CACHE_TAG(cp, prefetch_addr);
    md_addr_t prefetch_set = CACHE_SET(cp, prefetch_addr);
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  /* generate a prefetch request to mem addr 'addr + cache_line_size
     ONLY if cache line is not already present */
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  md_addr_t curr_addr_tag = CACHE_TAG(cp, addr);

  md_addr_t pc = get_PC();
  int lower_pc_bits = log_base2(sizeof(md_inst_t));
  assert(cp->rpt_t);
  int rpt_idx = (pc >> lower_pc_bits) % (NUM_RPT_ENTRIES_OPEN);

  enum rpt_state curr_state, new_state;
  int curr_stride, prev_stride;
  int miss_hist_idx, miss_hist_pref_idx;

  md_addr_t rpt_match_tag = cp->rpt_t[rpt_idx].tag;
  if(rpt_match_tag != curr_addr_tag) {
    /* no corresponding entry in the RPT; enter insn in RPT, evacuate another entry if RPT full */
    cp->rpt_t[rpt_idx].tag = curr_addr_tag;
    cp->rpt_t[rpt_idx].prev_addr = addr;
    cp->rpt_t[rpt_idx].state = INIT;
    cp->rpt_t[rpt_idx].stride = 0;

    /* prefetch from miss history table if entry is available */
    //for(miss_hist_idx = 0; miss_hist_idx < MISS_HIST_SIZE; miss_hist_idx++){
    //  if(cp->miss_hist_t[miss_hist_idx].addr == addr){
        //miss_hist_prefetch(cp, addr, miss_hist_idx);
    //  }
    //}
  }
  else {
    curr_state = cp->rpt_t[rpt_idx].state;
    prev_stride = cp->rpt_t[rpt_idx].stride;
    curr_stride = addr - cp->rpt_t[rpt_idx].prev_addr;
    switch (curr_state)
    {
      case INIT:
        if(prev_stride == curr_stride) {
          cp->rpt_t[rpt_idx].state = STEADY;
        }
        else {
          cp->rpt_t[rpt_idx].state = TRANS;
          cp->rpt_t[rpt_idx].stride = curr_stride;
        }
        break;
    ...
    new_state = cp->rpt_t[rpt_idx].state;
    if(new_state == STEADY) stride_update(cp, addr, curr_stride);
  }
}
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  int lower_pc_bits = log_base2(sizeof(md_inst_t));
  assert(cp->rpt_t);
  int rpt_idx = (pc >> lower_pc_bits) % (cp->prefetch_type);
  ...
  if(rpt_match_tag != curr_addr_tag) {
    ...
    new_state = cp->rpt_t[rpt_idx].state;
    if(new_state != NOPRED) stride_update(cp, addr, curr_stride);
  }
}
	     ...
	     int prefetch)		/* 1 if the access is a prefetch, 0 if it is not */
{
  ...
  if (prefetch == 0 ) {
    ...
    if(cp->prefetch_type == 2){
      //assert(cp->miss_hist_t);
      //cp->miss_hist_t[cp->miss_hist_head].addr = addr;
      //cp->miss_hist_t[cp->miss_hist_head].num_occurences += 1;
      //cp->miss_hist_head = (cp->miss_hist_head + 1)%MISS_HIST_SIZE;
    }
    /* ECE552 Assignment 4 - END CODE */
  }
  else {
